...
<!--
	This page is provided to customers to register for CabsOnline application.
...
<?php 
	...
	function ValidateEmail($emailid,$name)
	{
		...
		if(strpos($emailid, '@') > 0 && strpos($emailid, '.') > strpos($emailid, '@')+1 && strpos(strrev($emailid), '.') > 0)
		{
			$email_to = $emailid;
			$subject = "GET-TAXI";
			$message = "Dear ". $name . ", welcome to use GET-TAXI!";
			$header = "From Anjali-Pai";
